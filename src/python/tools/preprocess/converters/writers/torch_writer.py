...
class TorchWriter(object):
    ...
    def write_to_binary(
        ...
        if num_partitions > 1:
            ...
            # Binary mirror for fast C++ loading (P*P int64)
            train_offsets_bin_path = self.output_dir / Path(PathConstants.train_partition_offsets_bin_path)
            print("Train partition offsets (binary) written to:", PathConstants.train_partition_offsets_bin_path)
            np.asarray(train_edges_offsets, dtype=np.int64).tofile(train_offsets_bin_path)
        ...
        if node_partitions is not None:
            node_partitions_path = self.output_dir / Path(PathConstants.node_partitions_path)
            print("Node→partition mapping written to:", PathConstants.node_partitions_path)
            node_partitions.numpy().astype(np.int32).tofile(node_partitions_path)

        if partition_score is not None:
            partition_score_path = self.output_dir / Path(PathConstants.partition_score_path)
            print("Partition critical score written to:", PathConstants.partition_score_path)
            partition_score.numpy().astype(np.int64).tofile(partition_score_path)
