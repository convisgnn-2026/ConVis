...
std::tuple<vector<torch::Tensor>, vector<torch::Tensor>> getNodePartitionOrdering(NodePartitionOrdering node_partition_ordering, Indices train_nodes,
                                                                                  ...
                                                                                  int fine_to_coarse_ratio, int num_cache_partitions,
                                                                                  torch::Tensor node_to_partition = {}, torch::Tensor partition_offsets = {},
                                                                                  torch::Tensor partition_score = {});
...
// HiCab-CLO node partition ordering. Requires node_to_partition (per-node partition mapping),
// partition_offsets (P*P inter-partition edge weights), and partition_score (per-partition critical score).
std::tuple<vector<torch::Tensor>, vector<torch::Tensor>> getCustomNodePartitionOrdering(Indices train_nodes, int64_t total_num_nodes, int num_partitions,
                                                                                        int buffer_capacity, int fine_to_coarse_ratio, int num_cache_partitions,
                                                                                        torch::Tensor node_to_partition, torch::Tensor partition_offsets,
                                                                                        torch::Tensor partition_score);
