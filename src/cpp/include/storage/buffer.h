...
// Global structure for collecting disk I/O statistics
struct DiskIOStats {
    std::atomic<int64_t> total_read_count{0};        // Total number of reads
    std::atomic<int64_t> total_write_count{0};       // Total number of writes
    std::atomic<int64_t> total_read_bytes{0};        // Total read bytes
    std::atomic<int64_t> total_write_bytes{0};       // Total write bytes
    std::atomic<int64_t> swap_count{0};              // Number of swaps
    std::atomic<int64_t> total_read_time_us{0};      // Total read time (microseconds)
    std::atomic<int64_t> total_write_time_us{0};     // Total write time (microseconds)
    
    // Initial loading statistics (persistent across epochs)
    std::atomic<int64_t> initial_load_count{0};      // Number of partitions loaded initially
    std::atomic<int64_t> initial_load_bytes{0};      // Initial loading bytes
    std::atomic<int64_t> initial_load_time_us{0};    // Initial loading time (microseconds)
    ...
    void reset() {
        ...
        // 주의: initial_load 통계는 리셋하지 않음 (에포크 간 보존)
    }
    ...
    void reset_all() {
        ...
        initial_load_time_us = 0;
    }
    
    void print_stats() {
        SPDLOG_INFO("=== Disk I/O Statistics ===");
        SPDLOG_INFO("Initial load partition count: {}", initial_load_count.load());
        SPDLOG_INFO("Initial load bytes: {:.2f} MB", initial_load_bytes.load() / (1024.0 * 1024.0));
        SPDLOG_INFO("Initial load time: {:.2f} ms", initial_load_time_us.load() / 1000.0);
        SPDLOG_INFO("Total read count: {}", total_read_count.load());
        SPDLOG_INFO("Total write count: {}", total_write_count.load());
        SPDLOG_INFO("Total read bytes: {:.2f} MB", total_read_bytes.load() / (1024.0 * 1024.0));
        SPDLOG_INFO("Total write bytes: {:.2f} MB", total_write_bytes.load() / (1024.0 * 1024.0));
        SPDLOG_INFO("Total read time: {:.2f} ms", total_read_time_us.load() / 1000.0);
        SPDLOG_INFO("Total write time: {:.2f} ms", total_write_time_us.load() / 1000.0);
        SPDLOG_INFO("Partition swap count: {}", swap_count.load());
        SPDLOG_INFO("Average read size: {:.2f} KB", total_read_count > 0 ? total_read_bytes.load() / (1024.0 * total_read_count.load()) : 0);
        SPDLOG_INFO("Average write size: {:.2f} KB", total_write_count > 0 ? total_write_bytes.load() / (1024.0 * total_write_count.load()) : 0);
        SPDLOG_INFO("Average read time: {:.2f} ms", total_read_count > 0 ? total_read_time_us.load() / (1000.0 * total_read_count.load()) : 0);
        SPDLOG_INFO("Average write time: {:.2f} ms", total_write_count > 0 ? total_write_time_us.load() / (1000.0 * total_write_count.load()) : 0);
        if (initial_load_count > 0) {
            SPDLOG_INFO("Average initial load size: {:.2f} KB", initial_load_bytes.load() / (1024.0 * initial_load_count.load()));
            SPDLOG_INFO("Average initial load time: {:.2f} ms", initial_load_time_us.load() / (1000.0 * initial_load_count.load()));
        }
        SPDLOG_INFO("===========================");
    }
};

// Global I/O statistics object
extern DiskIOStats g_disk_io_stats;
