...
// Global disk I/O statistics object definition
DiskIOStats g_disk_io_stats;
...
// Method to read partition data from disk file into memory
// Performs efficient disk reading using Direct I/O
void PartitionedFile::readPartition(void *addr, Partition *partition) {
    ...
    // Update disk I/O statistics (read)
    g_disk_io_stats.total_read_count.fetch_add(1);
    g_disk_io_stats.total_read_bytes.fetch_add(partition->total_size_);
    g_disk_io_stats.total_read_time_us.fetch_add(read_duration);
    SPDLOG_DEBUG("Disk READ: partition {} ({:.2f} KB, {:.2f} ms)", partition->partition_id_, partition->total_size_ / 1024.0, read_duration / 1000.0);
...
void PartitionedFile::writePartition(Partition *partition, bool clear_mem) {
    ...
    // Update disk I/O statistics (write)
    g_disk_io_stats.total_write_count.fetch_add(1);
    g_disk_io_stats.total_write_bytes.fetch_add(partition->total_size_);
    g_disk_io_stats.total_write_time_us.fetch_add(write_duration);
    SPDLOG_DEBUG("Disk WRITE: partition {} ({:.2f} KB, {:.2f} ms)", partition->partition_id_, partition->total_size_ / 1024.0, write_duration / 1000.0);
...
void PartitionBuffer::load() {
    if (!loaded_) {
        ...
        // Start measuring initial loading time
        auto initial_load_start = std::chrono::high_resolution_clock::now();
        int64_t initial_load_bytes_total = 0;
        int initial_partitions_loaded = 0;
        
        SPDLOG_INFO("=== Initial Loading Started: Loading {} partitions ===", buffer_state_.size(0));
        ...
        for (int i = 0; i < buffer_state_.size(0); i++) {
            ...
            auto partition_load_start = std::chrono::high_resolution_clock::now();
            partitioned_file_->readPartition(buff_addr, partition);
            auto partition_load_end = std::chrono::high_resolution_clock::now();
            auto partition_load_time = std::chrono::duration_cast<std::chrono::milliseconds>(partition_load_end - partition_load_start).count();
            
            partition->present_ = true;
            partition->buffer_idx_ = i;
            num_nodes += partition->partition_size_;
            
            initial_load_bytes_total += partition->total_size_;
            initial_partitions_loaded++;
            
            SPDLOG_DEBUG("Initial loading partition {}: {:.2f} KB, {} ms", 
                        partition_id, partition->total_size_ / 1024.0, partition_load_time);
        }
        
        auto initial_load_end = std::chrono::high_resolution_clock::now();
        auto initial_load_total_time = std::chrono::duration_cast<std::chrono::microseconds>(initial_load_end - initial_load_start).count();
        
        // Update initial loading statistics
        g_disk_io_stats.initial_load_count.fetch_add(initial_partitions_loaded);
        g_disk_io_stats.initial_load_bytes.fetch_add(initial_load_bytes_total);
        g_disk_io_stats.initial_load_time_us.fetch_add(initial_load_total_time);
        
        SPDLOG_INFO("=== Initial Loading Completed: {} partitions, {:.2f} MB, {:.2f} ms ===", 
                   initial_partitions_loaded, initial_load_bytes_total / (1024.0 * 1024.0), initial_load_total_time / 1000.0);
...
void PartitionBuffer::performNextSwap() {
    ...
    g_disk_io_stats.swap_count.fetch_add(1);
    SPDLOG_DEBUG("Partition swapping #{}: evict {} partitions, admit {} partitions", g_disk_io_stats.swap_count.load(), evict_ids.size(), admit_ids.size());
