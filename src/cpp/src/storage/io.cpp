...
std::tuple<shared_ptr<Storage>, shared_ptr<Storage>, shared_ptr<Storage>> initializeHicabArtifacts(shared_ptr<StorageConfig> storage_config) {
    shared_ptr<Storage> node_partitions = nullptr;
    shared_ptr<Storage> partition_offsets = nullptr;
    shared_ptr<Storage> partition_score = nullptr;

    int64_t num_nodes = storage_config->dataset->num_nodes;

    // num_partitions lives on PartitionBufferOptions (features or embeddings storage).
    int num_partitions = -1;
    if (storage_config->features != nullptr) {
        auto opts = std::dynamic_pointer_cast<PartitionBufferOptions>(storage_config->features->options);
        if (opts != nullptr) num_partitions = opts->num_partitions;
    }
    if (num_partitions <= 0 && storage_config->embeddings != nullptr) {
        auto opts = std::dynamic_pointer_cast<PartitionBufferOptions>(storage_config->embeddings->options);
        if (opts != nullptr) num_partitions = opts->num_partitions;
    }
    if (num_partitions <= 0 || num_nodes <= 0) {
        return std::forward_as_tuple(node_partitions, partition_offsets, partition_score);
    }

    string node_partitions_file =
        storage_config->dataset->dataset_dir + PathConstants::nodes_directory + PathConstants::node_partitions_file + PathConstants::file_ext;
    string partition_offsets_file = storage_config->dataset->dataset_dir + PathConstants::edges_directory + PathConstants::training +
                                    PathConstants::partition_offsets_bin_file + PathConstants::file_ext;
    string partition_score_file =
        storage_config->dataset->dataset_dir + PathConstants::edges_directory + PathConstants::partition_score_file + PathConstants::file_ext;

    if (file_exists(node_partitions_file)) {
        node_partitions = std::make_shared<InMemory>(node_partitions_file, num_nodes, 1, torch::kInt32, torch::kCPU);
    }
    if (file_exists(partition_offsets_file)) {
        partition_offsets = std::make_shared<InMemory>(partition_offsets_file, (int64_t)num_partitions * num_partitions, 1, torch::kInt64, torch::kCPU);
    }
    if (file_exists(partition_score_file)) {
        partition_score = std::make_shared<InMemory>(partition_score_file, num_partitions, 1, torch::kInt64, torch::kCPU);
    }
...
shared_ptr<GraphModelStorage> initializeStorageLinkPrediction(shared_ptr<Model> model, shared_ptr<StorageConfig> storage_config, bool reinitialize, bool train,
    ...
    auto hicab = initializeHicabArtifacts(storage_config);
    storage_ptrs.node_partitions = std::get<0>(hicab);
    storage_ptrs.partition_offsets = std::get<1>(hicab);
    storage_ptrs.partition_score = std::get<2>(hicab);
