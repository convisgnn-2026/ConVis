...
std::tuple<vector<torch::Tensor>, vector<torch::Tensor>> getSequentialNodePartitionOrdering(Indices train_nodes, int64_t total_num_nodes, int num_partitions,
    ...
    if (num_train_partitions > buffer_capacity) {
        SPDLOG_ERROR("SEQUENTIAL mode requires buffer_capacity ({}) >= num_train_partitions ({})",
                     buffer_capacity, num_train_partitions);
        SPDLOG_ERROR("Please increase buffer_capacity or use DISPERSED mode");
        throw std::runtime_error("Insufficient buffer capacity for SEQUENTIAL node partition ordering");
    }

    Indices in_buffer = torch::arange(num_train_partitions, torch::kInt32);

    Indices on_disk;
    if (buffer_capacity > num_train_partitions) {
        Indices available_partitions = torch::arange(num_train_partitions, num_partitions, torch::kInt32);
        if (available_partitions.size(0) > 0) {
            available_partitions = available_partitions.index_select(0, torch::randperm(available_partitions.size(0), torch::kInt64));
            int32_t extra_capacity = buffer_capacity - num_train_partitions;
            int32_t num_extra = std::min(extra_capacity, (int32_t)available_partitions.size(0));
            on_disk = available_partitions.narrow(0, 0, num_extra);
        }
    }

    if (on_disk.defined() && on_disk.size(0) > 0) {
...
std::tuple<vector<torch::Tensor>, vector<torch::Tensor>> getCustomNodePartitionOrdering(
    Indices train_nodes, int64_t total_num_nodes, int num_partitions, int buffer_capacity,
    int fine_to_coarse_ratio, int num_cache_partitions) {

    SPDLOG_INFO("Generating Custom Node Partition Ordering optimized for enhanced partitioning");

    int64_t partition_size = ceil((double)total_num_nodes / num_partitions);
    torch::Tensor train_nodes_partition = train_nodes.divide(partition_size, "trunc");

    int64_t critical_threshold = std::max(1L, (int64_t)(total_num_nodes * 0.05));
    int num_coarse_partitions = num_partitions / fine_to_coarse_ratio;
    int num_active_coarse = num_coarse_partitions - num_cache_partitions;

    vector<torch::Tensor> buffer_states;

    std::vector<int32_t> cache_partitions;
    for (int i = 0; i < num_cache_partitions * fine_to_coarse_ratio; i++) {
        cache_partitions.push_back(i);
    }

    Indices active_coarse_parts = torch::arange(num_active_coarse, torch::kInt32) + num_cache_partitions;
    Indices in_buffer = active_coarse_parts.narrow(0, 0, buffer_capacity / fine_to_coarse_ratio - num_cache_partitions);
    Indices on_disk = active_coarse_parts.narrow(0, in_buffer.size(0), active_coarse_parts.size(0) - in_buffer.size(0));

    torch::Tensor initial_buffer = torch::cat({in_buffer + num_cache_partitions,
                                             torch::arange(num_cache_partitions, in_buffer.options())});
    buffer_states.push_back(initial_buffer);
    ...
    while (on_disk.size(0) > 0) {
        ...
        if (on_disk.size(0) > 0) {
            in_buffer[-1] = on_disk[0];
            torch::Tensor new_buffer = torch::cat({in_buffer, torch::arange(num_cache_partitions, in_buffer.options())});
            buffer_states.push_back(new_buffer);
            on_disk = on_disk.narrow(0, 1, on_disk.size(0) - 1);
        }
    }

    torch::Tensor fine_to_coarse_map = torch::zeros(num_partitions, torch::kInt32);

    for (int i = 0; i < num_cache_partitions * fine_to_coarse_ratio; i++) {
        fine_to_coarse_map[i] = i / fine_to_coarse_ratio;
    }

    for (int i = num_cache_partitions * fine_to_coarse_ratio; i < num_partitions; i++) {
        int coarse_idx = (i - num_cache_partitions * fine_to_coarse_ratio) / fine_to_coarse_ratio + num_cache_partitions;
        fine_to_coarse_map[i] = coarse_idx;
    }

    vector<torch::Tensor> train_nodes_per_buffer;
    torch::Tensor train_nodes_buffer_assignment = torch::zeros_like(train_nodes);
    auto train_nodes_partition_accessor = train_nodes_partition.accessor<int32_t, 1>();

    std::vector<std::vector<int>> partition_to_buffers(num_partitions);

    for (int buf_idx = 0; buf_idx < buffer_states.size(); buf_idx++) {
        auto buffer_state = buffer_states[buf_idx];
        auto buffer_state_accessor = buffer_state.accessor<int32_t, 1>();

        std::set<int32_t> fine_parts_in_buffer;
        for (int i = 0; i < buffer_state.size(0); i++) {
            int32_t coarse_part = buffer_state_accessor[i];
            for (int j = 0; j < fine_to_coarse_ratio; j++) {
                int32_t fine_part = coarse_part * fine_to_coarse_ratio + j;
                if (fine_part < num_partitions) {
                    fine_parts_in_buffer.insert(fine_part);
                    partition_to_buffers[fine_part].push_back(buf_idx);
                }
            }
        }
    }

    for (int64_t i = 0; i < train_nodes.size(0); i++) {
        int32_t partition_id = train_nodes_partition_accessor[i];
        if (partition_id >= num_partitions) {
            partition_id = num_partitions - 1;
        }

        if (!partition_to_buffers[partition_id].empty()) {
            train_nodes_buffer_assignment[i] = partition_to_buffers[partition_id][0];
        } else {
            train_nodes_buffer_assignment[i] = 0;
        }
    }

    for (int i = 0; i < buffer_states.size(); i++) {
        torch::Tensor nodes_in_buffer = train_nodes.masked_select(train_nodes_buffer_assignment == i);
        train_nodes_per_buffer.push_back(nodes_in_buffer);
    }

    vector<torch::Tensor> fine_buffer_states;
    for (auto& coarse_buffer : buffer_states) {
        torch::Tensor fine_buffer = torch::zeros(buffer_capacity, torch::kInt32);
        auto coarse_accessor = coarse_buffer.accessor<int32_t, 1>();

        for (int i = 0; i < coarse_buffer.size(0); i++) {
            int32_t coarse_part = coarse_accessor[i];
            for (int j = 0; j < fine_to_coarse_ratio; j++) {
                int idx = i * fine_to_coarse_ratio + j;
                if (idx < buffer_capacity) {
                    fine_buffer[idx] = coarse_part * fine_to_coarse_ratio + j;
                }
            }
        }
        fine_buffer_states.push_back(fine_buffer);
    }

    SPDLOG_INFO("Custom ordering complete: {} buffer states created", fine_buffer_states.size());
    return std::forward_as_tuple(fine_buffer_states, train_nodes_per_buffer);
}
