...
static std::unordered_map<std::string, double> dataloader_timing;
static std::mutex dataloader_timing_mutex;
...
void DataLoader::nextEpoch() {
    {
        std::lock_guard<std::mutex> lock(dataloader_timing_mutex);
        if (dataloader_timing["load_storage_calls"] > 0 || dataloader_timing["batch_init_calls"] > 0) {
            SPDLOG_INFO("=== EPOCH {} DATALOADER PERFORMANCE SUMMARY ===", epochs_processed_ + 1);

            if (dataloader_timing["load_storage_calls"] > 0) {
                SPDLOG_INFO("Storage Loading (called {:.0f} times):", dataloader_timing["load_storage_calls"]);
                SPDLOG_INFO("  Buffer Ordering: {:.2f}ms total", dataloader_timing["buffer_ordering"]);
                SPDLOG_INFO("  Initial Partition Loading: {:.2f}ms total", dataloader_timing["initial_loading"]);
                SPDLOG_INFO("  Subgraph Initialization: {:.2f}ms total", dataloader_timing["subgraph_init"]);
                SPDLOG_INFO("  Total loadStorage(): {:.2f}ms", dataloader_timing["total_load_storage"]);
            }

            if (dataloader_timing["batch_init_calls"] > 0) {
                SPDLOG_INFO("Batch Initialization (called {:.0f} times):", dataloader_timing["batch_init_calls"]);
                SPDLOG_INFO("  Total Active Items: {:.0f}", dataloader_timing["total_active_items"]);
                SPDLOG_INFO("  Total Batches Created: {:.0f}", dataloader_timing["total_batches_created"]);
                SPDLOG_INFO("  Total initializeBatches(): {:.2f}ms", dataloader_timing["batch_initialization"]);
                SPDLOG_INFO("  Avg per call: {:.2f}ms", dataloader_timing["batch_initialization"] / dataloader_timing["batch_init_calls"]);
            }

            auto graph_timing = graph_storage_->getTimingData();
            if (graph_timing.find("total_subgraph_update_calls") != graph_timing.end() &&
                graph_timing["total_subgraph_update_calls"] > 0) {
                SPDLOG_INFO("Graph Storage Loading Performance:");
                SPDLOG_INFO("  Bucket Preparation: {:.2f}ms avg, {:.2f}ms total ({:.0f} calls)",
                           graph_timing["bucket_preparation_avg"],
                           graph_timing["bucket_preparation_total"],
                           graph_timing["bucket_preparation_calls"]);
                SPDLOG_INFO("  Node Mapping: {:.2f}ms avg, {:.2f}ms total ({:.0f} calls)",
                           graph_timing["node_mapping_avg"],
                           graph_timing["node_mapping_total"],
                           graph_timing["node_mapping_calls"]);
                SPDLOG_INFO("  Edge Loading: {:.2f}ms avg, {:.2f}ms total ({:.0f} calls)",
                           graph_timing["edge_loading_avg"],
                           graph_timing["edge_loading_total"],
                           graph_timing["edge_loading_calls"]);
                SPDLOG_INFO("  Total Subgraph Update: {:.2f}ms avg, {:.2f}ms total ({:.0f} calls)",
                           graph_timing["total_subgraph_update_avg"],
                           graph_timing["total_subgraph_update_total"],
                           graph_timing["total_subgraph_update_calls"]);
                if (graph_timing.find("total_edges_loaded_total") != graph_timing.end()) {
                    SPDLOG_INFO("  Cumulative Edges Loaded: {:.0f}", graph_timing["total_edges_loaded_total"]);
                }
            }

            SPDLOG_INFO("===============================================");

            global_bucket_mapping_profiler.print_detailed_stats();

            dataloader_timing.clear();
            graph_storage_->resetTimingData();
            global_bucket_mapping_profiler.reset();
        }
    }
...
void DataLoader::initializeBatches(bool prepare_encode) {
    ...
    {
        std::lock_guard<std::mutex> lock(dataloader_timing_mutex);
        dataloader_timing["batch_initialization"] += batch_init_time;
        dataloader_timing["total_active_items"] += num_items;
        dataloader_timing["total_batches_created"] += batches_.size();
        dataloader_timing["batch_init_calls"] += 1;
    }
}
...
void DataLoader::setBufferOrdering() {
    ...
    if (learning_task_ == LearningTask::LINK_PREDICTION) {
        ...
        if (graph_storage_->useInMemorySubGraph()) {
            ...
            torch::Tensor node_to_partition_t;
            torch::Tensor partition_offsets_t;
            torch::Tensor partition_score_t;
            if (graph_storage_->storage_ptrs_.node_partitions != nullptr) {
                graph_storage_->storage_ptrs_.node_partitions->load();
                node_to_partition_t =
                    graph_storage_->storage_ptrs_.node_partitions->range(0, graph_storage_->storage_ptrs_.node_partitions->getDim0()).flatten(0, -1);
            }
            if (graph_storage_->storage_ptrs_.partition_offsets != nullptr) {
                graph_storage_->storage_ptrs_.partition_offsets->load();
                partition_offsets_t =
                    graph_storage_->storage_ptrs_.partition_offsets->range(0, graph_storage_->storage_ptrs_.partition_offsets->getDim0()).flatten(0, -1);
            }
            if (graph_storage_->storage_ptrs_.partition_score != nullptr) {
                graph_storage_->storage_ptrs_.partition_score->load();
                partition_score_t =
                    graph_storage_->storage_ptrs_.partition_score->range(0, graph_storage_->storage_ptrs_.partition_score->getDim0()).flatten(0, -1);
            }
            ...
            auto tup = getNodePartitionOrdering(
                ...
                node_to_partition_t, partition_offsets_t, partition_score_t);
            buffer_states_ = std::get<0>(tup);
...
void DataLoader::loadStorage() {
    ...
    double ordering_time = std::chrono::duration<double, std::milli>(ordering_end - ordering_start).count();

    auto loading_start = std::chrono::high_resolution_clock::now();
    graph_storage_->load();
    auto loading_end = std::chrono::high_resolution_clock::now();
    double loading_time = std::chrono::duration<double, std::milli>(loading_end - loading_start).count();
    ...
    double subgraph_time = std::chrono::duration<double, std::milli>(subgraph_end - subgraph_start).count();
    ...
    {
        std::lock_guard<std::mutex> lock(dataloader_timing_mutex);
        dataloader_timing["buffer_ordering"] += ordering_time;
        dataloader_timing["initial_loading"] += loading_time;
        dataloader_timing["subgraph_init"] += subgraph_time;
        dataloader_timing["total_load_storage"] += total_time;
        dataloader_timing["load_storage_calls"] += 1;
    }
