...
#include "storage/buffer.h"  // 디스크 I/O 통계를 위해 추가
...
void PipelineTrainer::train(int num_epochs) {
    ...
    for (int epoch = 0; epoch < num_epochs; epoch++) {
        ...
        // 에폭 종료 시점에 디스크 I/O 통계 출력
        g_disk_io_stats.print_stats();
    }
}
...
void SynchronousTrainer::train(int num_epochs) {
    ...
    for (int epoch = 0; epoch < num_epochs; epoch++) {
        ...
        // Reset disk I/O statistics (preserve initial load stats after first epoch)
        if (epoch == 0) {
            g_disk_io_stats.reset_all();  // First epoch: reset all including initial load stats
        } else {
            g_disk_io_stats.reset();      // Subsequent epochs: preserve initial load stats
        }
